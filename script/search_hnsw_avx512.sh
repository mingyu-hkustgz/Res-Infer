...
for K in {20,100}; do
  for data in "${datasets[@]}"; do
    ...
    for randomize in {0..1}; do
      ...
      ./cmake-build-debug/src/search_hnsw_avx512 -d ${randomize} -i ${index} -q ${query} -g ${gnd} -r ${res} -t ${trans} -k ${K} -s ${efSearch}
    done
    ...
    ./cmake-build-debug/src/search_hnsw_avx512 -d ${randomize} -i ${index} -q ${query} -g ${gnd} -r ${res} -t ${trans} -l ${linear} -k ${K} -s ${efSearch}
    ...
    randomize=7
    res="${result_path}/${data}_ad_hnsw_${randomize}.log"
    ./cmake-build-debug/src/search_hnsw_avx512 -d ${randomize} -i ${index} -q ${query} -g ${gnd} -r ${res} -t ${trans} -e ${sigma} -k ${K} -s ${efSearch}
    ...
    ./cmake-build-debug/src/search_hnsw_avx512 -d ${randomize} -i ${index} -q ${query} -b ${code} -g ${gnd} -r ${res} -t ${trans} -l ${linear} -k ${K} -s ${efSearch}
